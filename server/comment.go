...
func (comment Comment) QueryId(id int) Comment {
...
func (comment *Comment) insert() bool {
    ...
    if comment.contents["date"] == "" ||  comment.contents["date"] == "NULL"{
        ...
        t1, err := time.Parse("2006-01-02", comment.contents["date"])
        if err != nil {
            fmt.Println(err)
            return false
        }
...
func (comment *Comment) delete(){
	_, err := db.Exec("DELETE FROM comment where id = ?", comment.contents["id"])
	if err != nil {
		log.Println(err)
		return
	}
