...
func (likeContent *LikeContent) insert() bool {
    ...
    if likeContent.contents["date"] == "" ||  likeContent.contents["date"] == "NULL"{
        ...
        t1, err := time.Parse("2006-01-02", likeContent.contents["date"])
        if err != nil {
            fmt.Println(err)
            return false
        }
...
func (likeContent *LikeContent) delete(){
    _, err := db.Exec("DELETE FROM likeContent where user_id = ? & content_id = ?", likeContent.contents["user_id"], likeContent.contents["content_id"])
    if err != nil {
        log.Println(err)
        return
    }
