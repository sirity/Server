...
func (favor Favor) QueryFavor(userId, contentId string) *Favor {

    // Execute the query
    rows, err := db.Query("SELECT * FROM " + favorTable + " where user_id = ? AND content_id = ?", userId, contentId)
    if err != nil {
        panic(err.Error()) // proper error handling instead of panic in your app
    }
...
func (favor *Favor) insert() bool {
    ...
    if favor.contents["date"] == "" ||  favor.contents["date"] == "NULL"{
        ...
        t1, err := time.Parse("2006-01-02 15:04:05", favor.contents["date"])
        if err != nil {
            fmt.Println(err)
            return false
        }
...
func (favor *Favor) delete() bool{
    result, err := db.Exec("DELETE FROM favor where user_id = ? AND content_id = ?", favor.contents["user_id"], favor.contents["content_id"])
